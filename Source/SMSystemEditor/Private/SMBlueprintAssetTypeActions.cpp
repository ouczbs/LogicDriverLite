...
void FSMBlueprintAssetTypeActions::OpenAssetEditor(const TArray<UObject*>& InObjects, TSharedPtr<class IToolkitHost> EditWithinLevelEditor)
{
	...
	for (auto ObjIt = InObjects.CreateConstIterator(); ObjIt; ++ObjIt)
	{
		...
		{
			if (StateMachineBP->bIsNewlyCreated && FSMBlueprintEditorUtils::GetTopLevelStateMachineGraph(StateMachineBP) == nullptr)
			{
				USMBlueprintFactory::CreateGraphsForNewBlueprint(StateMachineBP);
			}
