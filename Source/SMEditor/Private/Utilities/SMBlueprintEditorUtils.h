...
#include "Config/SMEditorSettings.h"
...
class SMEDITOR_API FSMBlueprintEditorUtils : public FBlueprintEditorUtils
{
public:
	...
	/** Recursively search all children graphs for a blueprint constructing a full list of all nodes matching the type. */
	template<typename T>
	...
	{
		if (T* ExistingNode = GetSingleNodeFromGraph<T>(Graph))
		{
			if (OutNode)
			{
	...
	static bool PlaceFunctionOnGraph(UEdGraph* Graph, UFunction* Function, UEdGraphPin* DestinationPin, UEdGraphNode** FunctionNodeOut,
		UEdGraphPin** FunctionArgumentPinOut, float WidthOffset = -100.f, bool bAutoWireObjects = true);
	...
	template<typename T>
	static T* GetSingleNodeFromGraph(UEdGraph* Graph)
	{
		TArray<T*> Nodes;
		GetAllNodesOfClassNested<T>(Graph, Nodes);
	...
	static UK2Node_CallParentFunction* CreateParentFunctionCall(UEdGraph* Graph, UFunction* ParentFunction, UEdGraphNode* ChildNode, int32 XPostion = 200, bool bIsDefault = true);
	...
	template<typename T>
	static T* ConvertNodeTo(USMGraphNode_Base* OriginalNode)
	{
